...
### Registro 27: Gerenciamento do Loop de Eventos, Estruturas de Protocolo e Controle E/S via Epoll (`ipc_server.c`, `ipc_server.h`)

Para otimizar o fluxo de I/O assíncrono não-bloqueante, estruturar a camada de resposta IPC e garantir a recuperação graciosa de conexões e encerramento do servidor, implementou-se o monitoramento dinâmico de eventos via `epoll_ctl`, o dimensionamento estático de buffers e a definição do cabeçalho de resposta da aplicação.

**Solução:**
* **Dimensionamento de Buffers e Protocolo de Resposta (`ipc_server.h`):** Definição das constantes de capacidade estática `RX_BUFFER_SIZE` (2048 bytes) para acumulação de comandos de entrada e `TX_BUFFER_SIZE` (32768 bytes) para comportar payloads de resposta compostos por múltiplos registros. Estruturação da `ipc_response_header_t` contendo os campos escalares `status` (int32_t) e `count` (uint32_t), padronizando a meta-informação enviada no início de cada quadro de transmissão antes do payload bruto de dados.
* **Controle Dinâmico de Escrita e Tratamento de Eventos (`ipc_server.c`):** Implementação da rotina auxiliar `update_epoll_events` para modificar os eventos monitorados do file descriptor (`EPOLL_CTL_MOD`) com base no status do buffer de transmissão (`tx_buffer`). A função registra persistentemente as flags de leitura e erro em modo Edge-Triggered (`EPOLLIN | EPOLLET | EPOLLRDHUP`) e habilita dinamicamente a flag de escrita (`EPOLLOUT`) apenas quando há pendência de flushing (`tx_offset < tx_bytes`). No loop principal da `ipc_server_start`, reformulou-se o laço de dispatching para tratar erros de socket e desconexões prematuras (`EPOLLRDHUP`, `EPOLLHUP`, `EPOLLERR`) de forma isolada, invocando a remoção do interesse (`EPOLL_CTL_DEL`) e a destruição do contexto (`client_destroy`) antes de despachar eventos de leitura (`handle_client_data`) ou escrita (`handle_client_write`). Por fim, garantiu-se o descarte seguro de recursos (*cleanup*) ao término da execução do loop, invocando o fechamento explícito dos file descriptors (`epoll_fd` e `server_fd`) e a remoção do arquivo de soquete UNIX do sistema de arquivos via `unlink`.

---

### Registro 28: Implementação da Camada Dupla de Segurança IPC - DAC e Handshake com Token Estático (`ipc_server.c`, `protocol.h` e `py_test.py`)

Para blindar o Unix Domain Socket contra acessos não autorizados no nível do sistema operacional e na camada de aplicação, implementou-se uma arquitetura de segurança em profundidade (*Defense in Depth*) combinando permissões nativas do Kernel Linux (DAC) com validação lógica por token estático em fluxo *fail-fast*.

**Solução:**
* **Controle de Acesso Discrecionário por Kernel - DAC (`ipc_server.c`):** Inclusão da syscall `chmod(socket_path, 0660)` imediatamente após a rotina de `bind()` no Unix Socket. Essa abordagem restringe as permissões de leitura e escrita do inode (`/tmp/coreauth.sock`) estritamente ao usuário e grupo do processo, fazendo com que o próprio Kernel Linux rejeite conexões de outros processos do sistema operacional com o erro `EACCES` (*Permission Denied*) sem consumir ciclos de CPU do motor em C.
* **Handshake e Autenticação de Aplicação (`protocol.h`, `ipc_server.c`):** Criação da estrutura empacotada `auth_protocol_t` e inclusão do opcode `OP_AUTH (0)`. A rotina `parse_and_execute_commands` foi adaptada para exigir que o primeiro pacote enviado por uma nova conexão seja obrigatoriamente a credencial de autenticação. O servidor realiza a comparação de memória do token recebido com a variável `TOKEN_AUTH` injetada via ambiente. Caso o token seja válido, a conexão é marcada como autenticada (`is_authenticated = true`); caso contrário, o servidor retorna um erro de status e invoca `client_destroy()` para encerrar a conexão imediatamente antes de processar qualquer instrução CRUD.
* **Carregamento Seguro e Falha Concreta:** Eliminação de tokens padrão (*hardcoded*) no código-fonte. A inicialização do motor C foi configurada para ler a chave do ambiente (`TOKEN_AUTH` / `.env`) em padrão *fail-fast*, abortando a execução na inicialização caso a variável não esteja definida.

---

### Registro 29: Correção de Race Condition (TOCTOU), Escalação de Privilégios no Socket IPC e Ajuste de Diretório de Execução (`ipc_server.h`, `ipc_server.c`)

Identificação de uma janela de concorrência (*Time-of-Check to Time-of-Use* - TOCTOU) na sequência de inicialização do Unix Socket, onde o uso de diretórios compartilhados (como `/tmp`) e a alteração tardia de permissões via `chmod()` permitiam que um atacante local criasse um *symlink* para redirecionar a alteração de permissões a arquivos críticos do sistema. Além disso, a tentativa de criação de pastas em `/run` por chamadas diretas a `mkdir` no código C causaria falhas de permissão (`EACCES`) em processos iniciados sob usuários restritos sem privilégios de root.
